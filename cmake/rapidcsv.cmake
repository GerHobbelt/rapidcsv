...
function(rapidcsv_cmake_variables_config)
    ...
    add_custom_target(genexdebug_rapidcsv_5 ALL COMMAND ${CMAKE_COMMAND} -E echo "**** ${_CNV_COMPILE_LANGUAGE_}")
    add_custom_target(genexdebug_rapidcsv_6 ALL COMMAND ${CMAKE_COMMAND} -E echo "**** ${_CNV_LINK_LANGUAGE_}")
    add_custom_target(genexdebug_rapidcsv_7 ALL COMMAND ${CMAKE_COMMAND} -E echo "**** ${_CNV_BUILD_ENV_}")
    add_custom_target(genexdebug_rapidcsv_8 ALL COMMAND ${CMAKE_COMMAND} -E echo "**** ${_CNV_ENV_MSYSTEM_}")
endfunction()
...
add_custom_target(genexdebug_rapidcsv_9 ALL COMMAND ${CMAKE_COMMAND} -E echo "**** OS: unix_like_os=${unix_like_os} , unix_os=${unix_os} , apple_os=${apple_os} , windows_os=${windows_os}")
...
add_custom_target(genexdebug_rapidcsv_10 ALL COMMAND ${CMAKE_COMMAND} -E echo "**** ENV-flag: mingw_build_env=${mingw_build_env} , msys_build_env=${msys_build_env} , cygwin_build_env=${cygwin_build_env}")
...
#add_custom_target(genexdebug_rapidcsv_11 ALL COMMAND ${CMAKE_COMMAND} -E echo "**** ENV: clang_cxx=${clang_cxx} , clang_like_cxx=${clang_like_cxx} , gcc_cxx=${gcc_cxx} , gcc_like_cxx=${gcc_like_cxx} , msvc_cxx=${msvc_cxx}")
...
function(rapidcsv_getversion version_arg)
    ...
    if (CMAKE_VERSION VERSION_GREATER_EQUAL 3.15)
        message(DEBUG "rapidcsv version ${VERSION}")
    else()
        message(STATUS "rapidcsv version ${VERSION}")
    endif()
...
function(rapidcsv_use_unicode)
    find_package(PkgConfig)
    pkg_check_modules(ICU REQUIRED icu-uc)

    target_link_libraries(rapidcsv INTERFACE ${ICU_LDFLAGS})
    target_compile_options(rapidcsv INTERFACE ${ICU_CFLAGS})
    target_compile_definitions(rapidcsv INTERFACE RAPIDCSV_USE_UNICODE)
endfunction()
#]==================================================================================]
...
#]===========]
macro(rapidcsv_check_cxx_compiler_flag_file_prefix_map)
    set(cxx_compiler_file_prefix_map OFF)
    if(CMAKE_BUILD_TYPE MATCHES "Coverage")
        message(STATUS "rapidcsv : compiler option '-ffile-prefix-map=old=new' skipped in 'Coverage' build, as it messes the gcovr file paths during header file search")
    else()
...
macro(rapidcsv_build)
    ...
    add_custom_target(genexdebug_rapidcsv_12 ALL COMMAND ${CMAKE_COMMAND} -E echo "**** ENV_MSYSTEM=${ENV_MSYSTEM}  , windows_os=${windows_os}")
    ...
    add_custom_target(genexdebug_rapidcsv_13 ALL COMMAND ${CMAKE_COMMAND} -E echo "**** MSYSTEM_VALUE=${MSYSTEM_VALUE}")
    target_compile_definitions(rapidcsv INTERFACE
    ...
    target_compile_features(rapidcsv INTERFACE
        cxx_constexpr
    ...
    message(STATUS "'rapidcsv' linking to '${CONVERTERLIB}'")
        ...
        # when <target> is a library (i.e NOT an executable), then specify the scope.
            target_link_libraries(<target>
                        ...
                        [<PRIVATE|PUBLIC|INTERFACE> <item>...]...)
        #]======================]
    target_link_libraries(rapidcsv INTERFACE ${CONVERTERLIB})
...
#        BASE_DIRS  $<BUILD_INTERFACE:${CMAKE_CURRENT_LIST_DIR}/include> $<INSTALL_INTERFACE:${CMAKE_INSTALL_INCLUDEDIR}>  #include
        FILES
            $<BUILD_INTERFACE:${CMAKE_CURRENT_LIST_DIR}/include>$<INSTALL_INTERFACE:${CMAKE_INSTALL_INCLUDEDIR}>/rapidcsv/rapidcsv.h
            $<BUILD_INTERFACE:${CMAKE_CURRENT_LIST_DIR}/include>$<INSTALL_INTERFACE:${CMAKE_INSTALL_INCLUDEDIR}>/rapidcsv/view.h
            $<BUILD_INTERFACE:${CMAKE_CURRENT_LIST_DIR}/include>$<INSTALL_INTERFACE:${CMAKE_INSTALL_INCLUDEDIR}>/rapidcsv/Properties.h
    )
endmacro(rapidcsv_build)
	...
	#[===================================================================[
	   installation
	#]===================================================================]
function(rapidcsv_install_logic)
    ...
    set(RAPIDCSV_CMAKE_DIR "${CMAKE_INSTALL_LIBDIR_ARCHIND}/cmake/rapidcsv"
    ...
    set(version_config "${PROJECT_BINARY_DIR}/rapidcsv-config-version.cmake")
    set(project_config "${PROJECT_BINARY_DIR}/rapidcsv-config.cmake")
    ...
    #]==================================================================================]
    set(targets_export_name rapidcsvTargets)    # targets_export_name used in packaging/rapidcsv-config.cmake.in
    ...
    configure_package_config_file(
        ${PackagingTemplatesDir}/rapidcsv-config.cmake.in      # donot change "rapidcsv-config.cmake" to ${project_config}
        ${project_config}
        INSTALL_DESTINATION ${RAPIDCSV_CMAKE_DIR})

    # Install version, config and target files.
    install(
        FILES       ${project_config} ${version_config}
        DESTINATION ${RAPIDCSV_CMAKE_DIR})
	...
	if( RAPIDCSV_ENABLE_INSTALL )
        ...
        # Uninstall
        add_custom_target(uninstall_rapidcsv COMMAND "${CMAKE_COMMAND}" -E remove "${CMAKE_INSTALL_PREFIX}/include/rapidcsv")
	endif()

endfunction(rapidcsv_install_logic)
