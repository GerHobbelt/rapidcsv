...
int main()
{
  ...
  try
  {
    ...
    std::variant<int, std::string> val_i = doc.GetCell<int, rapidcsv::ConvertFromStr_gNaN<int> >(0, 1);
    unittest::ExpectEqual(size_t, val_i.index(),1);
    unittest::ExpectEqual(std::string, std::get<std::string>(val_i), std::string(""));

    std::variant<long long, std::string> val_ll = doc.GetCell<long long, rapidcsv::ConvertFromStr_gNaN<long long> >(1, 1);
    unittest::ExpectEqual(size_t, val_ll.index(),1);
    unittest::ExpectEqual(std::string, std::get<std::string>(val_ll), std::string("y"));

    std::variant<unsigned int, std::string> val_ui = doc.GetCell<unsigned int, rapidcsv::ConvertFromStr_gNaN<unsigned int> >(2, 1);
    ...
    std::variant<double, std::string> val_d = doc.GetCell<double, rapidcsv::ConvertFromStr_gNaN<double> >(0, 1);
    unittest::ExpectEqual(size_t, val_d.index(),1);
    unittest::ExpectEqual(std::string, std::get<std::string>(val_d), std::string(""));

    std::variant<long double, std::string> val_ld = doc.GetCell<long double, rapidcsv::ConvertFromStr_gNaN<long double> >(1, 1);
    unittest::ExpectEqual(size_t, val_ld.index(),1);
    unittest::ExpectEqual(std::string, std::get<std::string>(val_ld), std::string("y"));

    std::variant<float, std::string> val_f = doc.GetCell<float, rapidcsv::ConvertFromStr_gNaN<float> >(2, 1);
